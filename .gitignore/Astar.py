...
class node():
        ...
        def get_f(self, current_node, end_node):
        ...
        for i in range(4):
                ...
                if 0 <= current_y and current_y < 30 and 0 <= current_x and current_x < 40:
                        if map_point[current_y][current_x].value:
                                li.append(map_point[current_y][current_x])
        return li

def draw_blue(): # 경로 그리기
        global list_BLUE, list_SKYBLUE,end_node,start_node
        current_node = end_node.parent
        cnt = 1
        while  not current_node == start_node:
                pygame.draw.rect(ourScreen, BLUE, pygame.Rect(int(current_node.point[1]*25 + 1), int(current_node.point[0]*25 + 1), 23, 23))
                if node in list_SKYBLUE:
                        list_SKYBLUE.remove(node)
                list_BLUE.add(current_node)
                current_node = current_node.parent
                cnt += 1
        pygame.display.flip()
        return cnt

def search_bfs(map_point):
    search_time = time.time()
    global start_node,end_node, memory
    open_list = list() # 지나야 할 노드
    close_list = list() # 지나온 노드
    current_node = start_node
    open_list.append(current_node)
    while True:
            current_node = open_list.pop(0)
            if current_node == end_node:
                    retime = time.time() - search_time
                    memory = sys.getsizeof(open_list) + sys.getsizeof(close_list)
                    return retime
            close_list.append(current_node)
            for node in node4(current_node, map_point):
                    if node in close_list or node in open_list : # 닫힌 목록에 있을 때
                            continue
                    else: #닫힌, 열린 목록에 없을 때
...
def search_dfs(map_point):
    ...
    while True:
        ...
        if current_node == end_node:
                ...
                return retime
        close_list.append(current_node)
        for node in node4(current_node, map_point): # 인접한 노드 반환
                if node in close_list or node in open_list: continue
                else: #닫힌, 열린 목록에 없을 때
                    node.parent = current_node
                    open_list.append(node)
...
def search(map_point): # 맵 탐색
    ...
    start_node.get_h(end_node)
    start_node.get_g(-1)


    open_list.append(current_node) # 현재노드를 열린 목록에 넣기
    ...
    while open_list:
        ...
        for node in node4(current_node, map_point): # 인접한 노드 반환
            if node in close_list or node in open_list: # 닫힌 목록에 있을 때
                ...
                node.get_f(current_node, end_node)
                open_list.append(node)
            '''
        for node in node4(current_node, map_point):
            if node in close_list: # 닫힌 목록에 있을 때
                    continue
            elif node in open_list : # 열린 목록에 있을 때
